...
void ForceField::init() {
	...
	Lx_coarse = int(Lx / delta)+1;
	Ly_coarse = int(Ly / delta)+1;
...
void ForceField::calc_force_field(BaseField *p) {
...
string ForceField::force_field(llint step) {
	stringstream conf;
	conf.precision(15);
	...
	conf<<"\n";
	for(int i=0; i<Ly_coarse; i++){
		for(int j=0; j<Lx_coarse; j++){
			int site = j + i * Lx_coarse;
			conf << (j * delta + delta/2) <<" "<<  (i * delta + delta/2)  << " " << f_field_coarse_x[site] << " " << f_field_coarse_y[site] << " "<< " "<< f_field_passive_coarse_x[site]<<" "<<f_field_passive_coarse_y[site]<<" "<<f_field_active_coarse_x[site]<<" "<<f_field_active_coarse_y[site]<<" ";
		}
	}
