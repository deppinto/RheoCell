...
void ShapeField::get_settings(input_file &my_inp, input_file &sim_inp) {
...
string ShapeField::headers(llint step) {
...
string ShapeField::field(BaseField *p) {
	stringstream conf;
	conf.precision(15);
...
void ShapeField::calc_field(BaseField *p) {

	for(int q=0;q<p->subSize; q++){
		int k = p->map_sub_to_box[q];

		f_field_x[k] += p->S00 * p->fieldScalar[q];
		f_field_y[k] += p->S01 * p->fieldScalar[q];
		phi_field[k] += p->fieldScalar[q];
	}
}

string ShapeField::f_field(llint step) {
	stringstream conf;
	conf.precision(15);
...
string ShapeField::get_output_string(llint curr_step) {
