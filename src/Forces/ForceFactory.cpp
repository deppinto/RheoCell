...
void ForceFactory::add_force(input_file &inp, std::vector<BaseField *> &fields, BaseBox * box_ptr) {
	...
	else if(type_str.compare("kolmogorov_flow_channel") == 0) extF = std::make_shared<KolmogorovFlowChannel>();
	else if(type_str.compare("pois_flow_channel") == 0) extF = std::make_shared<PoiseuilleFlowChannel>();
	else if(type_str.compare("stretching") == 0) extF = std::make_shared<Stretching>();
